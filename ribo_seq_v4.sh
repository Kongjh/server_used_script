...
for fac in `seq 0 0`
do 
	dirname=${factorlist[fac]}# 也就是 factor 的名字
	echo '-----------------${dirname}--------------------' # 用来标识到哪了
	...
	namelist=(`cat ${f}namelist.txt`)#都是 1-2-3 ribo; 1-2-3 mRNA
	filelist=(`cat ${f}seqlist/${dirname}/filelist.txt`)#这里就每个都不一样了
...
#time bowtie2-build -f /Dell/Dell4/kongjh/ribo-seq/test/index/rna_coding.fasta rna 
    	...
    	awk -F "\t" '{print $1,$10,$3,$NF,$4,$2,$6}' OFS="\t" ${ftitle}all-0-16-mapped-${name}.sam > ${ftitle}${name}.txt
